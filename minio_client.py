...
class MinioClient(object):
    ...
    def bucket_list_files(self, bucket_name, prefix):
        ...
        try:
            ...
            for obj in objects:
                object_name = obj.object_name.replace("video_", "")
                image_obj_name = object_name.replace(".mp4", ".jpeg")
                image_url = self.presigned_get_file(bucket_name, "img_" + image_obj_name, 1, response_headers)
                video_url = self.presigned_get_file(bucket_name, "video_" + object_name, 1, response_headers)
                file_list.append(
                    {"bucketName": obj.bucket_name, "imageUrl": image_url, "videoUrl": video_url, "content": object_name, "last_modified": obj.last_modified, "etag": obj.etag, "content-type": obj.content_type}
                )
        except S3Error as e:
            print("[error]:", e)
        return file_list
    ...
    def upload_byte_file(self, bucket_name, file, file_data, file_size, content_type):
        """
        上传文件 + 写入
        :param bucket_name: 桶名
        :param file: 文件名
        ...
        """
        try:
            self.client.put_object(bucket_name, file, file_data, file_size, content_type=content_type)
        except S3Error as e:
            print("[error]:", e)
    ...
    def presigned_get_file(self, bucket_name, file, days=7, response_headers=None):
        """
        生成一个http GET操作 签证URL
        :return:
        """
        return self.client.presigned_get_object(bucket_name, file, expires=timedelta(days=days), response_headers=response_headers)


if __name__ == '__main__':
    minio_obj = MinioClient(service="10.0.0.70:9000", access_key="AKIAIOSFODNN7EXAMPLE", secret_key="wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY")
